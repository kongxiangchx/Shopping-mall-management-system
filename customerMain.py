...
class Viewgoods(QMainWindow, Ui_view_goods):
    ...
    def click2(self):
        ...
        data = {'id': 'customer', 'type': 'view_goods', 'goods_name': goods_name,
                'goods_type': goods_type, 'shop_name': shop_name}
        s = Send_data()
        recv = s.message(data)
        s.close()
        l1 = len(recv['result'])
...
class Buygoods(QMainWindow, Ui_buy_goods):
    ...
    def click2(self):
        ...
        if trade_num <= 0:
            ...
            return
        data = {'id': 'customer', 'type': 'buy_goods', 'user': user, 'goods_name': goods_name,
                'shop_acc': shop_acc, 'trade_num': trade_num, 'trade_money': trade_money}
        # print(json.dumps(data))
        s = Send_data()
        recv = s.message(data)
        s.close()
...
class Viewtrade(QMainWindow, Ui_view_trade):
    ...
    def click2(self):
        goods_name = self.goods_name.text()
        goods_type = self.goods_type.currentText()
        data = {'id': 'customer', 'type': 'view_trade', 'user': user,
