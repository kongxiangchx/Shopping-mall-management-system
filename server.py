...
class Shop:
    ...
    def viewgoods(self,recv):
        op1=Shop_op()
        data={'result':op1.view_goods(recv['user'],recv['goods_name'],recv['goods_type'])}
        self.conn.send(json.dumps(data).encode())
        self.conn.close()
    def viewtrade(self,recv):
        op1=Shop_op()
        data={'result':op1.view_trade(recv['user'],recv['goods_name'],recv['goods_type'])}
        self.conn.send(json.dumps(data).encode())
        self.conn.close()
    ...
    def selectgoodsinfo(self,recv):
        op1=Shop_op()
        #data={'result':'success'}
        data={'result':op1.select_goodsinfo(recv['user'],recv['goods_name'])}
        self.conn.send(json.dumps(data).encode())
        self.conn.close()
...
class Customer:
    ...
    def viewgoods(self,recv):
        op1=Customer_op()
        data={'result':op1.view_goods(recv['goods_name'],recv['goods_type'],recv['shop_name'])}
        self.conn.send(json.dumps(data).encode())
        self.conn.close()
