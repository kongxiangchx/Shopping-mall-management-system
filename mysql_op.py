...
class Shop_op:
    ...
    def register(self,user,passwd,shop_name,phone,addr):
        ...
        if data is not None:
            self.close()
            return False
        value=[user,passwd,shop_name,phone,addr,time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))]
    ...
    def login(self,user,passwd):
        self.connect()
        value=[user,passwd]
        self.cur.execute('select shop_acc,shop_pass from shop where shop_acc=%s and shop_pass=%s',value)
        data=self.cur.fetchone()
        if data is not None:
            self.close()
            return True
    ...
    def add_goods(self,user,goods_name,goods_type,goods_prices,goods_rest):
        self.connect()
        self.cur.execute('select goods_name from goods where shop_acc=%s and goods_name=%s',[user,goods_name])
        data=self.cur.fetchone()
        if data is not None:
            self.close()
            return False
    ...
    def view_goods(self,user,goods_name,goods_type):
        self.connect()
        if goods_type=='全部':
            self.cur.execute('select goods_name,goods_type,goods_prices,goods_rest,goods_selled from goods where shop_acc=%s and goods_name like %s',[user,'%'+goods_name+'%'])
        else:
            self.cur.execute('select goods_name,goods_type,goods_prices,goods_rest,goods_selled from goods where shop_acc=%s and goods_name like %s and goods_type=%s',[user,'%'+goods_name+'%',goods_type])
    ...
    def update_goods(self,user,old_goods_name,goods_name,goods_type,goods_prices,goods_rest):
        self.connect()
        try:
            value=[goods_name,goods_type,goods_prices,goods_rest,user,old_goods_name]
            self.cur.execute('update goods set goods_name=%s,goods_type=%s,goods_prices=%s,goods_rest=%s where shop_acc=%s and goods_name=%s',value)
            self.conn.commit()
    ...
    def shop_info(self,user):
        self.connect()
        self.cur.execute('select shop_acc,shop_pass,shop_name,shop_phone,shop_addr,shop_time from shop where shop_acc=%s',[user])
        data=self.cur.fetchone()
        self.close()
        return data
    def update_shop(self,user,passwd,shop_name,phone,addr):
        ...
        try:
            ...
            self.cur.execute('update shop set shop_pass=%s,shop_name=%s,shop_phone=%s,shop_addr=%s where shop_acc=%s',value)
            self.conn.commit()
        except Exception as e:
            self.conn.rollback()
            self.close()
...
class Customer_op:
    ...
    def register(self,user,passwd,cus_name,phone,addr):
        self.connect()
        self.cur.execute('select cus_acc from customer where cus_acc=%s',[user])
        data=self.cur.fetchone()
        if data is not None:
            self.close()
            return False
    ...
    def login(self,user,passwd):
        self.connect()
        value=[user,passwd]
        self.cur.execute('select cus_acc,cus_pass from customer where cus_acc=%s and cus_pass=%s',value)
        data=self.cur.fetchone()
        if data is not None:
            self.close()
            return True
    ...
    def view_goods(self,goods_name,goods_type,shop_name):
        self.connect()
        if goods_type=='全部':
            self.cur.execute('select goods.goods_name,goods_type,shop_name,goods_prices,goods_rest from goods join shop on goods.shop_acc=shop.shop_acc where goods.goods_name like %s and shop_name like %s',['%'+goods_name+'%','%'+shop_name+'%'])
        else:
            self.cur.execute('select goods.goods_name,goods_type,shop_name,goods_prices,goods_rest from goods join shop on goods.shop_acc=shop.shop_acc where goods.goods_name like %s and shop_name like %s and goods_type=%s',['%'+goods_name+'%','%'+shop_name+'%',goods_type])
        data=self.cur.fetchall()
        self.close()
        return data
    def all_goods(self):
        self.connect()
        self.cur.execute('select goods.goods_name,goods_type,shop_name,goods_prices,goods_rest,shop.shop_acc from goods join shop on goods.shop_acc=shop.shop_acc')
        data=self.cur.fetchall()
        self.close()
        return data
    def buy_goods(self,user,goods_name,shop_acc,trade_num,trade_money):
        self.connect()
        self.cur.execute('select goods_rest from goods where shop_acc=%s and goods_name=%s',[shop_acc,goods_name])
        data=self.cur.fetchone()
        #print(data)
        if data is None or data[0]-int(trade_num)<0:
            ...
            return False
        try:
            value=(user,shop_acc,goods_name,trade_num,trade_money,time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
    ...
    def cus_info(self,user):
        self.connect()
        self.cur.execute('select cus_acc,cus_pass,cus_name,cus_phone,cus_addr from customer where cus_acc=%s',[user])
        data=self.cur.fetchone()
        self.close()
        return data
    def update_cus(self,user,passwd,cus_name,phone,addr):
        ...
        try:
            ...
            self.cur.execute('update customer set cus_pass=%s,cus_name=%s,cus_phone=%s,cus_addr=%s where cus_acc=%s',value)
            self.conn.commit()
        except Exception as e:
            self.conn.rollback()
            self.close()
